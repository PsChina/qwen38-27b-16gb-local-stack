...
try {
    ...
    if ($health.StatusCode -eq 200 -and $props.modalities.vision -eq $true -and $props.model_alias -eq 'Qwen3.8-27B-Q3') {
        Write-Host "Q3 llama-server is already vision-ready on $BackendHost`:$BackendPort"
        exit 0
    }
} catch {
